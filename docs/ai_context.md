...
- **Journal entry ledger layout** - Full rewrite of `templates/finance/journal.html`. Uses 4-column CSS Grid (`48px 1fr 64px 64px`) with always-present grid children (code|name|debit|credit). Description column removed from main ledger line — treated as secondary metadata in memo footer. Per-entry column header row, compact row padding, mobile-responsive. Page container now max-width 900px (not --wide) for tighter layout. See `docs/feature-journal-entries.md`.
